    ...
    with exclusive_lock(lock_path):
        ...
        if existing:
            ...
            if receipt_path.is_file():
                ...
                original = _capture_receipt(state_dir, existing, "created")
            ...
            original["subject"] = "english"
            original["capture_id"] = existing["event_id"]
            original["capture_content_sha256"] = object_sha256(existing)
            atomic_write_json(receipt_path, original)
        ...
        receipt = _capture_receipt(state_dir, proposed, "created")
...
def _capture_receipt(state_dir: Path, event: dict[str, Any], status: str) -> dict[str, Any]:
    ...
    return {
        ...
        "capture_content_sha256": event_hash,
        "status": status,
